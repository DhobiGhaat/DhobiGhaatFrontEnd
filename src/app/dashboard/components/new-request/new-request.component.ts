...
export class NewRequestComponent implements OnInit {
    ...
    curDate = (new Date().getFullYear()).toString() + "-0" + (new Date().getMonth() + 1).toString() + "-" + (new Date().getDate()).toString();

    order: Order = new Order("", "", "", this.curDate, "ORDER_PLACED", "", "");
    ...
    NewRequest() {
      ...
      this.svc.CreateNewRequest(this.order)
      .subscribe(
        data => {
